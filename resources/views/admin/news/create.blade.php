...
<head>
    ...
    <title>Tambah Berita Baru - Intellecta Admin</title>
...
<body class="bg-gray-50 font-sans antialiased">
    <div class="min-h-screen flex">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Tambah Berita Baru</h1>
                <p class="text-sm text-gray-500 mt-1">Buat berita atau pengumuman baru untuk diakses oleh siswa.</p>
            </div>
                    ...
                    <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="8" required
                                      ...
                                      class="block w-full px-4 py-2.5 border @error('content') border-red-300 @else border-gray-200 @enderror rounded-xl text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent hover:border-purple-300 transition-all duration-200">{{ old('content') }}</textarea>
                            @error('content')
                                <p class="text-xs text-red-500 font-medium mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Image File -->
                        <div>
                            <label for="image" class="block text-sm font-bold text-gray-700 mb-1.5">Gambar Cover (Opsional)</label>
                            ...
                            <small class="text-gray-400 text-xs mt-1 block">Format: JPG, JPEG, PNG, GIF. Maksimal 2MB.</small>
                            @error('image')
                                <p class="text-xs text-red-500 font-medium mt-1">{{ $message }}</p>
                            @enderror
                        </div>
