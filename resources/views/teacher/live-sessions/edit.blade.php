...
<head>
    ...
    <title>Intellecta - Edit Kelas Live</title>
...
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <!-- Header Section -->
            <div class="mb-8">
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Jadwal Kelas Live</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui detail pertemuan kelas tatap muka online Anda.</p>
            </div>

            <!-- Form Card Layout -->
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm max-w-2xl overflow-hidden mb-12">
                <div class="p-6 sm:p-8">
                    <form action="{{ route('teacher.live-sessions.update', $session) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <label class="relative flex items-center justify-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-purple-200 hover:bg-purple-50/10 active:scale-95 transition-all duration-150">
                                    <input type="radio" name="platform" value="google_meet" required {{ old('platform', $session->platform) == 'google_meet' ? 'checked' : '' }} class="sr-only peer">
                                ...
                                <label class="relative flex items-center justify-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-purple-200 hover:bg-purple-50/10 active:scale-95 transition-all duration-150">
                                    <input type="radio" name="platform" value="zoom" {{ old('platform', $session->platform) == 'zoom' ? 'checked' : '' }} class="sr-only peer">
                                    <div class="text-center peer-checked:text-purple-600">
                                        <div class="text-xl mb-0.5">🔵</div>
                                ...
                                <label class="relative flex items-center justify-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-purple-200 hover:bg-purple-50/10 active:scale-95 transition-all duration-150">
                                    <input type="radio" name="platform" value="microsoft_teams" {{ old('platform', $session->platform) == 'microsoft_teams' ? 'checked' : '' }} class="sr-only peer">
                                    <div class="text-center peer-checked:text-purple-600">
                                        <div class="text-xl mb-0.5">🟣</div>
                                ...
                                <label class="relative flex items-center justify-center p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-purple-200 hover:bg-purple-50/10 active:scale-95 transition-all duration-150">
                                    <input type="radio" name="platform" value="other" {{ old('platform', $session->platform) == 'other' ? 'checked' : '' }} class="sr-only peer">
                                    <div class="text-center peer-checked:text-purple-600">
                                        <div class="text-xl mb-0.5">🔗</div>
                        ...
                        <div>
                            ...
                            <input type="url" name="meeting_link" id="meeting_link" required value="{{ old('meeting_link', $session->meeting_link) }}"
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="scheduled_at" id="scheduled_at" required value="{{ old('scheduled_at', $localScheduledAt) }}"
                            ...
                            <div>
                                ...
                                <select name="duration_minutes" id="duration_minutes" required
                                    ...
                                    <option value="30" {{ old('duration_minutes', $session->duration_minutes) == '30' ? 'selected' : '' }}>30 Menit</option>
                                    <option value="45" {{ old('duration_minutes', $session->duration_minutes) == '45' ? 'selected' : '' }}>45 Menit</option>
                                    <option value="60" {{ old('duration_minutes', $session->duration_minutes) == '60' ? 'selected' : '' }}>60 Menit (1 Jam)</option>
                                    <option value="90" {{ old('duration_minutes', $session->duration_minutes) == '90' ? 'selected' : '' }}>90 Menit (1.5 Jam)</option>
                                    <option value="120" {{ old('duration_minutes', $session->duration_minutes) == '120' ? 'selected' : '' }}>120 Menit (2 Jam)</option>
                                    <option value="180" {{ old('duration_minutes', $session->duration_minutes) == '180' ? 'selected' : '' }}>180 Menit (3 Jam)</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="max_participants" class="block text-sm font-bold text-gray-700 mb-1.5">Kuota Peserta Maksimal</label>
                                <input type="number" name="max_participants" id="max_participants" min="1" value="{{ old('max_participants', $session->max_participants) }}"
                            ...
                            <div>
                                <label for="status" class="block text-sm font-bold text-gray-700 mb-1.5">Status Pertemuan <span class="text-red-500">*</span></label>
                                <select name="status" id="status" required
                                        class="block w-full px-4 py-2.5 border @error('status') border-red-300 @else border-gray-200 @enderror rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent hover:border-purple-300 transition-all duration-200">
                                    ...
                                    <option value="live" {{ old('status', $session->status) == 'live' ? 'selected' : '' }}>LIVE / Sedang Berlangsung</option>
